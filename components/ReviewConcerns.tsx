    ...
    <div className="rounded-3xl border border-white/10 bg-white/[0.045] p-6 shadow-2xl shadow-black/25 backdrop-blur-xl">
      ...
      <div className="space-y-3">
        {concerns.map((concern) => (
          <div key={`${concern.author}-${concern.file ?? concern.state}-${concern.concern}`} className="rounded-2xl border border-white/10 bg-slate-950/55 p-4">
            <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
              <span className="font-medium text-slate-300">@{concern.author}</span>
            ...
            <p className="text-sm leading-6 text-slate-300">{concern.concern}</p>
          </div>
        ))}
      </div>
    </div>
